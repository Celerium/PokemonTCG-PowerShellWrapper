<#
    ...
    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\baseUri\Remove-PokemonTCGBaseURI.Tests.ps1

        Runs a pester test and outputs simple results

    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\baseUri\Remove-PokemonTCGBaseURI.Tests.ps1 -Output Detailed
...
Describe "Testing [ $commandName ] function with [ $pester_TestName ]" -Tag @('baseUri') {
    ...
    Context "[ $commandName ] testing function" {
        ...
        It "The baseUri variable should not longer exist" {
            ...
            $PokemonTCG_BaseURI | Should -BeNullOrEmpty
        }

        It "If the baseUri is already gone a warning should be thrown" {
            Add-PokemonTCGBaseURI
            Remove-PokemonTCGBaseURI
            Remove-PokemonTCGBaseURI -WarningAction SilentlyContinue -WarningVariable baseUriWarning
